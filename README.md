...
アプリには**3Dモデル自動差し替えフック**を実装済みです。

1. リグ済みglTFモデルを `assets/soupy.glb` に配置
2. http(s) でアプリを配信（`python3 -m http.server` でOK。file://では3Dは無効）
3. ホーム/そだてるのヒーローが自動で `<model-viewer>`（vendor同梱）の3D表示に切替わり、ドラッグで回転・アイドルアニメーション再生
